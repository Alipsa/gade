...
public class MainMenu extends MenuBar {
  ...
  public void displayFind() {
    ...
    scene.getStylesheets().addAll(Gride.instance().getStyleSheets());
  ...
  private Menu createHelpMenu() {
    ...
    MenuItem about = new MenuItem("About Gride");
  ...
  private void viewLogFile(ActionEvent actionEvent) {
    try {
      ...
      Map.Entry<String, Appender> appenderEntry = logger.get().getAppenders().entrySet().stream()
          .filter(e -> "GrideLog".equals(e.getKey())).findAny().orElse(null);
      if (appenderEntry == null) {
        Alerts.warn("Failed to find log file", "Failed to find an appender called GrideLog");
        return;
      }
