...
import se.alipsa.gade.code.bashtab.BashTab;
...
import se.alipsa.gade.code.pythontab.PythonTab;
...
public class CodeComponent extends BorderPane {
  ...
  public TextAreaTab addCodeTab(CodeType type) {
    ...
    TextAreaTab tab = switch (type) {
      ...
      case BASH -> new BashTab(type.getDisplayValue(), gui);
      case PYTHON -> new PythonTab(type.getDisplayValue(), gui);
  ...
  public TextAreaTab addTab(File file, CodeType type) {
    ...
    try {
      ...
      tab = switch (type) {
        ...
        case BASH -> new BashTab(title, gui);
        case PYTHON -> new PythonTab(title, gui);
