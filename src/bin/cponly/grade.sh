...
if [[ "${OS}" == "win" ]]; then
  if [[ -z "$JAVA_CMD" ]]; then
	...
	start "${BIN_DIR}\${JAVA_CMD}" -cp "${LIB_DIR}/${JAR_NAME}" se.alipsa.grade.splash.SplashScreen
	# shellcheck disable=SC2068
	start "${BIN_DIR}\${JAVA_CMD}" -Djava.library.path="${LD_PATH}" -cp "${CLASSPATH}" $JAVA_OPTS se.alipsa.grade.Grade

else
	JAVA_CMD="java"
	CLASSPATH="${JAR_NAME}:${LIB_DIR}/*"
	LD_PATH="${LIB_DIR}"
	"${BIN_DIR}/${JAVA_CMD}" -cp "${LIB_DIR}/${JAR_NAME}" se.alipsa.grade.splash.SplashScreen &
	# shellcheck disable=SC2068
	"${BIN_DIR}/${JAVA_CMD}" -Djava.library.path="${LD_PATH}" -cp "${CLASSPATH}" $JAVA_OPTS se.alipsa.grade.Grade &
fi